...
public class CheckmuteCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String @NotNull [] args) {
        if(command.getName().equalsIgnoreCase("checkmute")) {
            if(args.length == 0) {
                SentinelMC.instance.getMessageUtil().sendMessage(commandSender, "mutesystem.checkmute.usage");
                return false;
            }
            ...
            SentinelMC.instance.getMessageUtil().sendMessage(commandSender, "mutesystem.checkmute.header", Map.of("player", targetProfile.getName()));
            SanctionManager manager = SentinelMC.instance.getSanctionManager();
            Punishment punishment = manager.getActiveMute(targetProfile.getUuid());
            if(punishment == null) {
                SentinelMC.instance.getMessageUtil().sendMessage(commandSender, "mutesystem.checkmute.not_muted", Map.of("player", targetName));
            } else {
                ...
                List<String> checkMessage = SentinelMC.instance.getMessageUtil().getMultiLineMessage("mutesystem.checkmute.muted", placeholders);
