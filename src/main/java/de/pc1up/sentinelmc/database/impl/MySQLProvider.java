...
public class MySQLProvider implements DatabaseProvider {
    ...
    public void initialize() {
        try {
            String host = SentinelMC.instance.getConfiguration().getString("database.credentials.mysql.host", "localhost");
            int port = SentinelMC.instance.getConfiguration().getInt("database.credentials.mysql.port", 3306);
            String database = SentinelMC.instance.getConfiguration().getString("database.credentials.mysql.database", "sentinelmc");
            String user = SentinelMC.instance.getConfiguration().getString("database.credentials.mysql.user", "sentinelmc");
            String password = SentinelMC.instance.getConfiguration().getString("database.credentials.mysql.password", "");

            String url = "jdbc:mysql://" + host + ":" + port + "/" + database + "?useSSL=false&autoReconnect=true";
            connection = DriverManager.getConnection(url, user, password);
            SentinelMC.instance.getLogger().info("[SentinelMC] MySQL connected!");
    ...
    public void savePunishment(Punishment p) {
        try (PreparedStatement ps = connection.prepareStatement("""
                    REPLACE INTO sentinelmc_punishments 
    ...
    public void saveUser(UserProfile u) {
        try (PreparedStatement ps = connection.prepareStatement("""
                    REPLACE INTO sentinelmc_userprofiles (uuid, name, lastIp, points)
                    VALUES (?, ?, ?, ?);
                """)) {
